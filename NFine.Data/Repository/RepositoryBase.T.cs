...
namespace NFine.Data
{
    ...
    public class RepositoryBase<TEntity> : IRepositoryBase<TEntity> where TEntity : class, new()
    {
        ...
        public List<TEntity> FindList(Expression<Func<TEntity, bool>> predicate, string orders)
        {
            bool isAsc = false;
            string[] _order = orders.Split(',');
            MethodCallExpression resultExp = null;
        ...
        public List<TEntity> FindList(Expression<Func<TEntity, bool>> predicate, Pagination pagination)
        {
            ...
            foreach (string item in _order)
            {
                ...
                _orderPart = Regex.Replace(_orderPart, @"\s+"," ");
                char[] chs = {' '};
                string[] _orderArry = _orderPart.Split(chs, StringSplitOptions.RemoveEmptyEntries);//把分割出的null值给remove掉
                string _orderField = _orderArry[0];
                bool sort = isAsc;
                if (_orderArry.Length == 2)
                {
