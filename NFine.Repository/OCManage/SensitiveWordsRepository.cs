...
{
    public class SensitiveWordsRepository : RepositoryBase<SensitiveWordsEntity>, ISensitiveWordsRepository
    {
        public void DeleteForm(string keyValue)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                ...
                foreach (string i in keys)
                    db.Delete<SensitiveWordsEntity>(t => t.F_Id == i);
                db.Commit();
            }
        ...
        public void SubmitForm(SensitiveWordsEntity sensitiveWordsEntity, string keyValue)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                ...
                {
                    db.Update(sensitiveWordsEntity);
                }
                else
                {
                    db.Insert(sensitiveWordsEntity);
                }
                db.Commit();
            }
        }
