...
namespace NFine.Web.Areas.OCManage.Controllers
{
    public class BaseChannelController : ControllerBase
    {
        ...
        LogEntity AddChannellogEntity = new LogEntity();
        LogEntity ChangeChannellogEntity = new LogEntity();
        LogEntity DeleteChannellogEntity = new LogEntity();
        ...
        public ActionResult newSubmitForm(BaseChannelEntity basechannelEntity, ChannelProvinceEntity channelprovinceEntity, ChannelConfigEntity channelconfigEntity, string keyValue)
        {
            ...
            AddChannellogEntity.F_ModuleName = "添加基础通道";
            AddChannellogEntity.F_Type = DbLogType.Create.ToString();
            ChangeChannellogEntity.F_ModuleName = "修改基础通道";
            ChangeChannellogEntity.F_Type = DbLogType.Update.ToString();
            ChangeChannellogEntity.F_Account = AddChannellogEntity.F_Account = OperatorProvider.Provider.GetCurrent().UserCode;
            ChangeChannellogEntity.F_NickName = AddChannellogEntity.F_NickName = OperatorProvider.Provider.GetCurrent().UserName;
            ...
            if (!string.IsNullOrEmpty(keyValue))//修改通道
            {
                ...
                try
                {
                    ...
                    ChangeChannellogEntity.F_Result = true;
                    ChangeChannellogEntity.F_Description = "修改基础通道成功";
                    new LogApp().WriteDbLog(ChangeChannellogEntity);
                }
                catch (Exception ex)
                {
                    ChangeChannellogEntity.F_Result = false;
                    ChangeChannellogEntity.F_Description = "修改基础通道失败," + ex;
                    new LogApp().WriteDbLog(ChangeChannellogEntity);
            ...
            else//新建通道
            {
                ...
                try
                {
                    ...
                    AddChannellogEntity.F_Result = true;
                    AddChannellogEntity.F_Description = "添加基础通道成功";
                    new LogApp().WriteDbLog(AddChannellogEntity);
                }
                catch (Exception ex)
                {
                    AddChannellogEntity.F_Result = false;
                    AddChannellogEntity.F_Description = "添加基础通道失败" + ex;
                    new LogApp().WriteDbLog(AddChannellogEntity);
                    return Error("操作失败，请刷新页面重试" + ex);
                }
            }
            return Success("操作成功。");
        ...
        private void ValueMove(OC_ChannelConfig model_CC, OC_BaseChannel model_BC, BaseChannelEntity basechannelEntity, ChannelConfigEntity channnelconfigEntity)
        {
            ...
            if (basechannelEntity.F_unsubscribe == ""||basechannelEntity.F_unsubscribe== "&nbsp;")
                model_BC.F_unsubscribe = null;
            ...
            model_BC.F_autographContent = basechannelEntity.F_autographContent;
            //给model_CC赋值
            ...
            model_CC.F_Version = channnelconfigEntity.F_Version;
            model_CC.F_CurrentSpeed = channnelconfigEntity.F_CurrentSpeed;
        }
        ...
        {
            DeleteChannellogEntity.F_ModuleName = "删除基础通道";
            DeleteChannellogEntity.F_Type = DbLogType.Delete.ToString();
            DeleteChannellogEntity.F_Account = AddChannellogEntity.F_Account = OperatorProvider.Provider.GetCurrent().UserCode;
            DeleteChannellogEntity.F_NickName = AddChannellogEntity.F_NickName = OperatorProvider.Provider.GetCurrent().UserName;
            try
            {
                BaseChannelApp.DeleteForm(keyValue);
                DeleteChannellogEntity.F_Result = true;
                DeleteChannellogEntity.F_Description = "删除基础通道成功";
                new LogApp().WriteDbLog(DeleteChannellogEntity);
            }
            catch(Exception ex) {
                DeleteChannellogEntity.F_Result = false;
                DeleteChannellogEntity.F_Description = "删除基础通道失败," + ex;
                new LogApp().WriteDbLog(DeleteChannellogEntity);
                return Error("删除失败:" + ex + "请联系管理员！");
            }
            try
            {
                DAL.OC_ChannelProvinceDAL.Instance.Delete(t => t.F_Id == keyValue);
            }
            catch (Exception ex)
            {
                return Error("通道已删除，但是出现错误，通道省份并未删除。错误:" + ex + "请联系管理员！");
            }
            return Success("联删成功！");
        }
        ...
        [HandlerAuthorize]
        [HandlerAjaxOnly]
        public ActionResult DeleteProvince(string F_Id,string ProvinceId)
        {
            ChannelProvinceApp channelapp = new ChannelProvinceApp();
            string []Province= ProvinceId.Split(',');
            Province = Province.Where(s => !string.IsNullOrEmpty(s)).ToArray();
            try
            {
                foreach (var item in Province)
                {
                    //找出省份Model，把通道Id改为0.表示删除
                    OC_ChannelProvince channelprovinceModel = DAL.OC_ChannelProvinceDAL.Instance.FindEntity(t => t.F_Id == F_Id && t.F_ProvinceId == item.ToInt());
                    channelprovinceModel.F_ChannelId = 0;
                    DAL.OC_ChannelProvinceDAL.Instance.Update(channelprovinceModel);
                }
            }
            catch(Exception e)//抛出异常
            { return Error("删除失败，请联系管理员！" + "============" + e); }
